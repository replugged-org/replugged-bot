...
async function loadCommand(command: string) {
  ...
  if (typeof cmd.executor !== 'function')
    throw new TypeError(`Invalid Command: ${basename(command)}`);
  bot.registerCommand(basename(command, '.js'), cmd.executor, {
    description: cmd.description,
    aliases: cmd.aliases,
  });
}

Promise.resolve(new MongoClient(process.env.MANGO || 'mongodb://127.0.0.1:27017'))
  ...
  .then((commands: string[]) =>
    Promise.all(commands.map(loadCommand)).catch((e) => console.error(e)),
  )
  .then(() => bot.connect())
