...
function isProcessable(msg: Message<GuildTextableChannel>): boolean {
  return (
    ...
    !(
      ...
      (!msg.embeds[0] || msg.embeds[0].type !== 'image')
    )
  );
}
...
function extractMedia(msg: Message<GuildTextableChannel>): {
  image?: EmbedImage;
  video?: EmbedVideo;
} {
...
async function updateStarCount(msg: Message<GuildTextableChannel>, count: number): Promise<void> {
  ...
  const entry = (await msg._client.mango
    .collection<StarboardEntry>('starboard')
    .findOne({ _id: msg.id })) || { ...GENERIC_STAR_OBJ };
  entry.stars = count;
  ...
  msg._client.mango
    .collection<StarboardEntry>('starboard')
    .updateOne({ _id: msg.id }, { $set: { ...entry } }, { upsert: true });
}
...
export default function (bot: CommandClient): void {
  bot.on('messageReactionAdd', (msg, emoji, user) =>
    process(msg as Message<GuildTextableChannel>, emoji as PartialEmoji, user as HasId),
  );
  bot.on('messageReactionRemove', (msg, emoji, user) =>
    process(msg as Message<GuildTextableChannel>, emoji as PartialEmoji, { id: user } as HasId),
  );
  bot.on('messageReactionRemoveAll', (msg) =>
    updateStarCount(msg as Message<GuildTextableChannel>, 0),
  );
